...
import com.example.matechatting.network.PagingHomeItemDataSourceFactory
...
class HomeItemRepository(private val homeItemDao: HomeItemDao) : BaseRepository {
    ...
    fun getHomeItemFromDB(callback: (LiveData<PagedList<HomeItemBean>>) -> Unit){
        val detailList = LivePagedListBuilder(homeItemDao.getHomeItem(),PagedList
            .Config.Builder()
    ...
    companion object {
        ...
        fun getInstance(homeItemDao: HomeItemDao) =
            instance ?: synchronized(this) {
                instance ?: HomeItemRepository(homeItemDao).also { instance = it }
            }
    }
}